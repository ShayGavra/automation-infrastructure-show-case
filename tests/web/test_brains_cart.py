...
class TestBrainsCart:
    ...
    def test05_increase_quantit(self,cart_flows:BrainsFlows):
       cart_flows.add_all_products_to_cart()
       cart_flows.navigate_to_cart()
       current_product_quantity =cart_flows.get_prodcut_quantity(0)
       cart_flows.increase_quantity(0,3)#PRODUCT_INDEDX,TIMES
       expected_product_quntity = current_product_quantity + 3
       updated_product_quntity = cart_flows.get_prodcut_quantity(0)
    ...
    def test06_verify_remove_products_from_cart(self,cart_flows:BrainsFlows):
       cart_flows.remove_products_from_cart()
       WebVerify.count(cart_flows.cart.remove_buttons, 0)
